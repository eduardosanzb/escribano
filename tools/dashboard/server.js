...
app.get('/api/observations', (req, res) => {
  try {
    ...
    if (recording_id) {
      whereClauses.push('recording_id = ?');
      params.push(recording_id);
    }
    ...
    const activityFilters = parseMultiParam(activity_type);
    if (activityFilters.length > 0) {
      const placeholders = activityFilters.map(() => '?').join(', ');
      whereClauses.push(`activity_type IN (${placeholders})`);
      params.push(...activityFilters);
    }

    const appFilters = parseMultiParam(app);
    appFilters.forEach(value => {
      whereClauses.push('apps LIKE ?');
      params.push(`%"${value}"%`);
    });

    if (search) {
      whereClauses.push('(vlm_description LIKE ? OR text LIKE ?)');
      const like = `%${search}%`;
    ...
    if (source === 'recorder') {
      whereClauses.push('frame_id IS NOT NULL');
    } else if (source === 'batch') {
      whereClauses.push('frame_id IS NULL');
    }
    ...
    const total = db
      .prepare(`SELECT COUNT(*) as count FROM observations WHERE ${whereSql}`)
      .get(...params).count;
