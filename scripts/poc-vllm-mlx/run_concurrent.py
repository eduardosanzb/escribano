#!/usr/bin/env python3
"""
mlx-vlm Concurrent POC - Test asyncio concurrent processing

Tests concurrent processing for multiple models with concurrency=4:
1. Qwen3-VL-2B-Instruct-4bit
2. Qwen3-VL-4B-Instruct-4bit
3. gemma-3n-E4B-it-bf16

Usage:
    uv run python run_concurrent.py
"""
...
from config import DB_PATH, TEST_FRAMES, TEMPERATURE, MAX_TOKENS, VLM_PROMPT, MODELS_TO_TEST, BASELINE_SECONDS_PER_FRAME
...
def test_model_concurrent(model_info: dict, frames: list, model_index: int, total_models: int) -> dict:
    """Test a single model with concurrent processing."""
    ...
    try:
        ...
        print(f"\n[2/3] Running concurrent benchmark...")
        result = benchmark.run_benchmark_concurrent(
            frames, VLM_PROMPT, CONCURRENCY, TEMPERATURE, MAX_TOKENS
        )
        ...
        output_path = f"docs/mlx-vlm-poc-concurrent-{model_slug}-{timestamp}.html"
...
def main():
    print("=" * 70)
    print("🚀 mlx-vlm Concurrent POC")
    print("=" * 70)
    print(f"\nTesting {len(MODELS_TO_TEST)} models with concurrency={CONCURRENCY}")
    ...
    for i, model_info in enumerate(MODELS_TO_TEST, 1):
        result = test_model_concurrent(model_info, frames, i, len(MODELS_TO_TEST))
    ...
    print("\n" + "=" * 70)
    print("📊 CONCURRENT COMPARISON SUMMARY")
    print("=" * 70)
